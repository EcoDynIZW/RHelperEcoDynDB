...
createProjPeople = function(){
  ...
  person_name = utils::select.list(c(sort(paste(f_people$family_name, f_people$first_name, sep = ", ")), "other"), graphics = F)
  ...
  # person details
  if(person_name != "other"){
    family_name = strsplit(person_name, ", ")[[1]][1]
    first_name = strsplit(person_name, ", ")[[1]][2]
  }
  people_id <<- people$people_id[paste(people$family_name, people$first_name, sep = ", ") == paste(family_name, first_name, sep = ", ")]
  person_name <<- paste(family_name, first_name, sep = ", ")
  ...
  add_aff = function(){
    ...
    if(organisation == "other"){
      ...
      country = utils::select.list(f_country$country_name_short, graphics=F)
    ...
    if((paste(people_id, org_id, year_of_aff) %in% paste(affs$people_id, affs$org_id, affs$year_of_aff)) == F){
     ...
     message(paste0("\n'", org, "' was added as affiliation for '", year_of_aff, "' for '", person_name,"."))
    } else {
      message(paste0("\nThe affiliation '", org, " ", year_of_aff, "' for '", person_name," already exists in the database."))
    }
  ...
  if(sec_aff == "NO"){
    ...
    message(paste0("\nYou entered all relevant affiliations of person '", person_name, "' and linked it to the project '", proj_name, "'.\nYou can always add additional affiliations by running the createProjPeople() function."))
  }
  ...
  add_role = function(){
    ...
    if(length(proj_role) > 1){
      message(paste0(paste0("'",proj_role,"'", collapse = ", "), " were added as role of '", person_name, "' in the project ",paste0("'",proj_name,"'", collapse = ", "), "."))
    }
    if(length(proj_role) == 1){
      message(paste0(paste0("'",proj_role,"'", collapse = ", "), " was added as role of '", person_name, "' in the projects ",paste0("'",proj_name,"'", collapse = ", "), "."))
    }
  }    
